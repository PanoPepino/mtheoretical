    ...
    def __init__(
        ...
        **kwargs,
        ...
        my_path = split_dictionary_path(the_dictionary)[
            0
        ]  # Uses function in refeq_funcs to split the relative path
        ...
        if the_equation in load_the_dic:
            self.chosen_equation = MathTex(
                load_the_dic[the_equation], font_size=text_size, color=text_color
            )
        ...
        # Personal modifications for a specific set of equations that require to appear together in my slides #
            ...
            if the_equation == "wave":
                p0 = MathTex(
                    str(load_the_dic["wave0"]), font_size=text_size, color=text_color
                )
                p1 = MathTex(
                    str(load_the_dic["wave1"]),
                    font_size=1.2 * text_size,
                    color=text_color,
                ).next_to(p0, RIGHT, buff=tightness)
                p2 = (
                    MathTex(
                        str(load_the_dic["wave2"]),
                        font_size=text_size,
                        color=text_color,
                    )
                    .next_to(p1, RIGHT, buff=tightness)
                    .shift(0.4 * UP)
                )
                p3 = MathTex(
                    str(load_the_dic["wave3"]), font_size=text_size, color=text_color
                ).next_to(p2, DOWN, aligned_edge=LEFT, buff=tightness)
                self.chosen_equation = VGroup(p0, p1, p2, p3)
            if the_equation == "prob boundaries":
                p0 = MathTex(
                    str(load_the_dic["prob hh"]), font_size=text_size, color=text_color
                )
                p1 = MathTex(
                    str(load_the_dic["prob vil"]), font_size=text_size, color=text_color
                ).next_to(p0, DOWN, aligned_edge=LEFT, buff=tightness)
                self.chosen_equation = VGroup(p0, p1)
            if the_equation == "prob comparison":
                p0 = MathTex(
                    str(load_the_dic["prob BT"]), font_size=text_size, color=text_color
                )
                p1 = MathTex(
                    str(load_the_dic["prob WKB"]), font_size=text_size, color=text_color
                ).next_to(p0, DOWN, aligned_edge=LEFT)
                self.chosen_equation = VGroup(p0, p1)
        ...
        if decorator_presence == "box":
            self.box = SurroundingRectangle(
                self.chosen_equation,
                corner_radius=list(corner_rad * np.array(corner_rad_direction)),
                buff=tightness,
                stroke_width=decorator_stroke_width,
                color=decorator_color,
                fill_opacity=fill_opa,
            )
