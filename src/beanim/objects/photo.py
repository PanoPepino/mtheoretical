...
class Photo(Group):
    ...
    .. attention:
        ...
        There is some issue with this class that requires to be fixed ASAP.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if decorator_style == "polaroid":
            ...
            r2 = (
                Rectangle(width=1.8, height=2.1)
                .move_to(r1.get_center())
                .shift(0.3 * UP)
            )
            polaroid = Cutout(
                r1, r2, fill_opacity=1, color=WHITE, stroke_color=GRAY_A
            ).scale(1.5)

            image = (
                ImageMobject(photo)
                .set(z_index=-1)
                .scale_to_fit_width(polaroid.get_width())
                .move_to(polaroid.get_center() + [0, 0.5, 0])
            )
            pin = (
                SVGMobject(get_svg_path)
                .scale(0.2)
                .next_to(polaroid, UP, buff=-0.1)
                .shift(0.2 * RIGHT)
            )
            ...
            # Text under polaroid
            texto = (
                Tex(caption, font_size=text_size, color=text_color)
                .next_to(polaroid, DOWN, buff=-0.7)
                .set(z_index=4)
            )
        ...
        if decorator_style == "techno":
            ...
            frame = SurroundingRectangle(
                ...
                stroke_width=2 * decorator_stroke_w,
                buff=0.03,
            )
