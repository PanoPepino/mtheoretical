...
__all__=['import_module_from_input']
...
def import_module_from_input():
    ...
    if module_name in allowed_modules:
        try:
            module = importlib.import_module("beanim.src.templates." + module_name) #It seems one has to specify all the way down to the module
            print("-------------------------------------------------------------------------------")
    ...
    if module_name not in allowed_modules:
        try:
            ...
            module = importlib.import_module("beanim.src.templates.template_0") #It seems one has to specify all the way down to the module
        except ImportError:
            return None
